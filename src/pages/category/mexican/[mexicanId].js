...
const MexicanRecipeDetails = ({TopRecipeDetails}) => {
  return (
    <MainContainer>
      <div className="recipe_details_wrapper">
          <div className={styles.reciepeHero}>
              ...
              <Link href="/category/mexican" className="back"></Link>
          </div>
...
export async function getServerSideProps(context){
  ...
  const responsTopRecipe = await fetch(`${env.apiUrlOfMexican}${params.mexicanId}`, {
    method: 'GET',
    headers: {
      'X-RapidAPI-Key': env.apiKey,
      'X-RapidAPI-Host': env.hostUrlOfMexican
    }
  });
