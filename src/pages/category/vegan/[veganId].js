...
const VeganRecipeDetails = ({TopRecipeDetails}) => {
  return (
    <MainContainer>
      <div className="recipe_details_wrapper">
          <div className={styles.reciepeHero}>
              ...
              <Link href="/category/vegan" className="back"></Link>
          </div>
...
export default VeganRecipeDetails
...
export async function getServerSideProps(context){
  ...
  const responsTopRecipe = await fetch(`${env.apiUrlOfVegan}${params.veganId}`, {
    method: 'GET',
    headers: {
      'X-RapidAPI-Key': env.apiKey,
      'X-RapidAPI-Host': env.hostUrlOfVegan
    }
  });
