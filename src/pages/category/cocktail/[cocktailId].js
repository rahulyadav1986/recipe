...
const CocktailRecipeDetails = ({TopRecipeDetails}) => {
  return (
    <MainContainer>
      <div className="recipe_details_wrapper">
          <div className={styles.reciepeHero}>
              ...
              <Link href="/category/cocktail" className="back"></Link>
          </div>
...
export async function getServerSideProps(context){
  ...
  const responsTopRecipe = await fetch(`${env.apiUrlOfCocktail}${params.cocktailId}`, {
    method: 'GET',
    headers: {
      'X-RapidAPI-Key': env.apiKey,
      'X-RapidAPI-Host': env.hostUrlOfCocktail
    }
  });
